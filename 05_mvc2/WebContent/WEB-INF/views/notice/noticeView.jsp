			...
			<table class="table" id="noticeView" style="width: 100%;">
				...
				<tr class="talbe-light">
					...
					<th colspan="3">
						...
						href="noticeDown?noticeNo=<%=n.getNoticeNo()%>"><%=n.getFilename()%></a>
						<%
						}
						%>
					</th>
			...
			<form action="/insertComment" method="POST">
				<ul>
					...
					<li><input type="hidden" name="ncLevel" value="1" /> <input
						type="hidden" name="ncWriter" value="<%=m.getMemberId()%>" /> <input
						type="hidden" name="noticeRef" value="<%=n.getNoticeNo()%>" /> <input
...
<script>
	$(".recShow").click(function() {
		var idx = $(".recShow").index(this);  //클릭한 게 몇번쨰 답글달기 버튼인지 구하는 코드
		$(this).hide();
	...
	function modifyCommentofComment(obj,ncNo,noticeNo,ncRef){
		//textarea를 보여줌
		console.log(obj);
		$(obj).parent().prev().show();
		//기존 본문 내용을 숨김
	 ...
	 function modifyCancel(obj, ncNo, noticeNo) {
	  ...
	  function modifyComplete(obj,ncNo,noticeNo){
		    var form = $("<form action='/noticeCommentUpdate' method='post'></form>");	//주의: 큰따옴표 안에서 따옴표를 표현하려면 작은 따옴표를 사용해야함
